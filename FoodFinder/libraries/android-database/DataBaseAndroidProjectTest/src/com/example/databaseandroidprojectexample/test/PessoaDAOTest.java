...
public class PessoaDAOTest extends AndroidTestCase {
	...
	public void testQueryByWhereClauseSelectionArgs() {
		...
		List<Pessoa> pessoas = PessoaDAO.getInstance(getContext()).getElementsByWhereClause("nome = ? and idade = ?", 
		new String[] {
				"Leonardo Viana",
	...
	public void testQueryByWhereClause() {
		...
		assertEquals(pessoas.size(), 1);
		//getElementsByWhereClause("nome = 'Leonardo Viana'");
		List<Pessoa> pessoasOutraQuery = PessoaDAO.getInstance(getContext()).getElements("nome = ?", new String[] {"Leonardo Viana"});
		assertEquals(pessoasOutraQuery.size(), 1);
